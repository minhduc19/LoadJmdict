...
class JMDictXMLParser(object):
    ...
    def parse_file(self):
        ...
        actual_path = os.path.abspath(os.path.expanduser("C:\\Users\\de925289\\Downloads\\PythonExperiment\\LoadJmdictToDatabase\\jamdict\\data\\JMdict_e"))
...
parseText = JMDictXMLParser()
#print(parseText.parse_file())
# Convert the object to a string representation
entries = parseText.parse_file()
#object_string = str(listOfObjects)
#json_string = json.dumps(entries.__dict__)


# Open the file in write mode and write the object string
...
# Create a dictionary mapping table names to their corresponding reflected class objects
#reflected_class_dict = {table_name: class_obj for table_name, class_obj in Base.classes.items()}

# Print the dictionary
#for table_name, class_obj in reflected_class_dict.items():
#    print(f"Table: {table_name}, Class: {class_obj}")

#print(reflected_class_dict.get("link"))
# Create variables dynamically based on table names
#for table_name, class_obj in Base.classes.items():
#    globals()[table_name] = class_obj
...
EntryLink = Base.classes.link
Bib = Base.classes.bib
Etym = Base.classes.etym
Kanji = Base.classes.kanji
Kji = Base.classes.kji
Kjp = Base.classes.kjp
Kana = Base.classes.kana
Kni = Base.classes.kni
Knp = Base.classes.knp
Knr = Base.classes.knr
SenseEntry = Base.classes.sense
Stagk = Base.classes.stagk
Stagr = Base.classes.stagr
Pos = Base.classes.pos
Xref = Base.classes.xref
Antonym = Base.classes.antonym
Field  = Base.classes.field
Misc = Base.classes.misc
SenseInfo = Base.classes.senseinfo
SenseSource = Base.classes.sensesource
Dialect = Base.classes.dialect
SenseGlossEntry = Base.classes.sensegloss
print(dir(Etym))

def insert_entry(entry):
    newEntry = Entry(idseq = entry.idseq)
    session.add(newEntry)
    if entry.info:
        # links
        for lnk in entry.info.links:
            newEntryLink = EntryLink(idseq = entry.idseq, tag = lnk.tag, description = lnk.desc, url = lnk.uri)
        # bibs
        for bib in entry.info.bibinfo:
            newBib = Bib(idseq = entry.idseq, tag = bib.tag, text = bib.text)
        # etym
        for e in entry.info.etym:
            newEtym = Etym(idseq = entry.idseq, text = e)
    # insert kanji
    for kj in entry.kanji_forms:
        newKanji = Kanji(idseq = entry.idseq, text = kj.text)
        session.add(newKanji)
        # KJI
        for kji in kj.info:
            newKji = Kji(text = kji, kid = newKanji.id, kanji = newKanji)
            session.add(newKji)

        # KJP
        for kjp in kj.pri:
            newKjp = Kjp(text = kjp, kid = newKanji.id, kanji = newKanji)
            session.add(newKjp)

     # insert kana
    for kn in entry.kana_forms:
        newKana = Kana(idseq = entry.idseq, text = kn.text, nokanji = kn.nokanji)
        session.add(newKana)
        # KNI
        for kni in kn.info:
            newKni = Kni(kid = newKana.id, text = kni,kana = newKana)
            session.add(newKni)
        # KNP
        for knp in kn.pri:
            newKnp = Knp(kid = newKana.id, text = knp,kana = newKana)
            session.add(newKnp)
        # KNR
        for knr in kn.restr:
            newKnr = Knr(kid = newKana.id, text = knr,kana = newKana)
            session.add(newKnr)
    # insert senses
    for s in entry.senses:
        NewSenseEntry = SenseEntry(idseq = entry.idseq)
        session.add(NewSenseEntry)
        # stagk
        for sk in s.stagk:
            NewStagk = Stagk(sid = NewSenseEntry.id, text = sk)
            session.add(NewStagk)
            print(sk)
        # stagr
        for sr in s.stagr:
            NewStagr = Stagr(sid = NewSenseEntry.id, text = sr)
            session.add(NewStagr)
        # pos
        for pos in s.pos:
            NewPos = Pos(sid = NewSenseEntry.id, text = pos)
            session.add(NewPos)
        # xref
        for xr in s.xref:
            NewXref = Xref(sid = NewSenseEntry.id, text = xr)
            session.add(NewXref)
        # antonym
        for a in s.antonym:
            NewAntonym = Antonym(sid = NewSenseEntry.id, text = a)
            session.add(NewAntonym)
        # field
        for f in s.field:
            NewField = Field(sid = NewSenseEntry.id, text = f)
            session.add(NewField)
        # misc
        for m in s.misc:
            NewMisc = Field(sid = NewSenseEntry.id, text = m)
            session.add(NewMisc)
        # SenseInfo
        for i in s.info:
            NewSenseInfo = Field(sid = NewSenseEntry.id, text = i)
            session.add(NewSenseInfo)
            print(i)
            print(NewSenseEntry.id)
        # SenseSource
        for l in s.lsource:
            NewSenseSource = SenseSource(sid = NewSenseEntry.id, text = l.text, lang = l.lang, lstype = l.lstype, wasei = l.wasei)
            session.add(NewSenseSource)

        # dialect
        for d in s.dialect:
            NewDialect = Dialect(sid = NewSenseEntry.id, text = d)
            session.add(NewDialect)
        # SenseGloss
        for g in s.gloss:
            NewSenseGlossEntry = SenseGlossEntry(sid = NewSenseEntry.id, lang = g.lang, gend = g.gend, text = g.text)
            session.add(NewSenseGlossEntry)
